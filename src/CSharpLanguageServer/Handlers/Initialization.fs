    ...
    let handleInitialize
        ...
        async {
            ...
            let windowShowMessage m =
                lspClient.WindowShowMessage({ Type = MessageType.Info; Message = m })
            ...
            logger.info (
                Log.setMessage
                    "{name} is released under MIT license and is not affiliated with Microsoft Corp.; see https://github.com/razzmatazz/csharp-language-server"
                >> Log.addContext "name" serverName
            )
...
*)
    ...
    let handleInitialized
        ...
        async {
            logger.trace (Log.setMessage "handleInitialized: \"initialized\" notification received from client")

            let registrationParams =
                { Registrations = getRegistrations context.ClientCapabilities |> List.toArray }
            ...
            logger.trace (Log.setMessage "handleInitialized: OK")
