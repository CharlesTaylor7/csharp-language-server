...
module InlayHint =
    ...
    let private getParameterForArgumentSyntax
        ...
        | :? BaseArgumentListSyntax as argumentList when not (isNull argumentList.Parent) ->
            let symbols =
                semanticModel.GetSymbolInfo(argumentList.Parent) |> getBestOrAllSymbols
            ...
            | [| symbol |] ->
                ...
                let namedParameter =
                    if isNull argument.NameColon || argument.NameColon.IsMissing then
                        ...
                        parameters
                        |> Seq.tryFind (fun p -> p.Name = argument.NameColon.Name.Identifier.ValueText)
    ...
    let provider
        ...
        : U3<bool, InlayHintOptions, InlayHintRegistrationOptions> option =
