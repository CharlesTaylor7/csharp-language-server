...
module Completion =
    ...
    let emptyRoslynOptionSet: Microsoft.CodeAnalysis.Options.OptionSet =
        ...
        let osEmptyOptionSetField =
            osType.GetField("Empty", BindingFlags.Static ||| BindingFlags.NonPublic)
    ...
    let private makeLspCompletionItem (item: Microsoft.CodeAnalysis.Completion.CompletionItem) (cacheKey: uint64) =
    ...
    let private cache =
        new LruCache<(Microsoft.CodeAnalysis.Document * Microsoft.CodeAnalysis.Completion.CompletionList)>(5)

    let handle
        (context: ServerRequestContext)
        (p: CompletionParams)
        : AsyncLspResult<Ionide.LanguageServerProtocol.Types.CompletionList option> =
        async {
            ...
            | Some doc ->
                ...
                return
                    ...
                    |> Option.map (fun completions ->
                        ...
                        let items =
                            completions.ItemsList |> Seq.map (flip makeLspCompletionItem key) |> Array.ofSeq
    ...
    let resolve (_context: ServerRequestContext) (item: CompletionItem) : AsyncLspResult<CompletionItem> = async {
        match
            ...
            |> Option.bind (fun (doc, cachedItems) ->
                ...
                |> Seq.tryFind (fun x ->
                    x.DisplayText = item.Label
                    && (item.SortText.IsNone || x.SortText = item.SortText.Value))
        ...
        | Some(doc, cachedItem) ->
            ...
            let itemDocumentation =
                description |> Option.map Documentation.fromCompletionDescription

            return
                { item with
                    Documentation = itemDocumentation |> Option.map U2.C2 }
                |> success
    }
